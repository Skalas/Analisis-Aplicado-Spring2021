...
def funcion_Rosenbrock(x,a=1,b=100):
    """
    Función de Rosenbrock, definida de R^2-->R, i.e. es una función real de dos variables
    """
    f = (a - x[0])**2 + b*(x[1] - x[0])**2
    return f

print(funcion_Rosenbrock([2,3])) #probamos si sirve la función 
print(funcion_Rosenbrock([1,1], 2, 3)) #probamos para otros parámetros a y b

#Ahora, copy-pasteo las funciones que ya tengo hechas que me van a servir para poder hacer lo que me piden
...
####Aquía acabo ya de copy pastear lo que hemos hecho en el curso y algunas tareas, a partir de ahora implementaré lo que pide el examen

#Respondemos el inciso de implementar Newton y tratar de optimizar

def Newton(f, x0):
    """
    Es sin usar alpha, la direccion de descenso es la inversa de la hessiana. Recibe una función y un punto inicial y su output es el minimizante de la función
    """
    xk = x0
...
print(Newton(funcion_Rosenbrock, [3,3]))
print(Newton(funcion_Rosenbrock, [2,7]))




#Respondemos el inciso de usar busqueda lineal para optimizar la funcion de Rosenbrock
def busqueda_lineal(f, x0, method = "Maximo descenso" ):
    """
    Busqueda_lineal recibe una función, en este caso recibirá la de Rosenbrock, un punto inicial y el método automático es el de máximo descenso, aunque se puede usar el de Newton
    """
    xk = x0  
    if method == "Newton" :
        while not (f_o_c(f,xk) and s_o_c(f,xk)):
...
print(busqueda_lineal(funcion_Rosenbrock, [3,3])) #Vemos que sí funciona el algoritmo! pues sabemos que el minimizante es [a,a^2]

# y estamos viendo que la respuesta da que el óptimo es [1.00000028 1.00000027] lo cual es [a,a^2] pues en este caso usamos a=1

print(busqueda_lineal(funcion_Rosenbrock, [7,5]))

#Lo que vemos es que ambas maneras (Newton y Busqueda lineal) resuelven lo que nos piden
#Sin embargo, es mucho más rápido el algoritmo de Newton, pues no debe calcular alpha y no pierde ese tiempo
#En este caso funcionó el algoritmo de Newton , pero con distintas funciones puede NO funcionar!
