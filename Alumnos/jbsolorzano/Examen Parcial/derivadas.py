...
def Hess(f, x0, h=1e-4, method = "centered"):
    """
    ...
    method: Método por el cual se quiere hacer puede ser: 'basic', 'grad',
    'centered', 'gradCentered'
    """
    n = len(x0)
...
def locochon(x):
    return np.sin(x[0]) ** 10 + np.cos(x[1] * x[0]) * np.cos(x[0])
